...
CIrrKlangAudioStreamMP3::CIrrKlangAudioStreamMP3(IFileReader *file)
    : File(file), TheMPAuDecContext(0), InputPosition(0), InputLength(0),
      DecodeBuffer(0), FirstFrameRead(false), EndOfFileReached(0), FileBegin(0),
      Position(0) {
  if (File) {
    File->grab();
    ...
    if (File->getSize() > 0) {
      ...
      while (!EndOfFileReached) {
        ...
        if (!EndOfFileReached /*&& File->isSeekable()*/) {
          ...
          data.offset = File->getPos() - (InputLength - InputPosition) -
...
SAudioStreamFormat CIrrKlangAudioStreamMP3::getFormat() { return Format; }
...
int CIrrKlangAudioStreamMP3::QueueBuffer::getSize() { return Size; }
...
void CIrrKlangAudioStreamMP3::QueueBuffer::clear() { Size = 0; }
