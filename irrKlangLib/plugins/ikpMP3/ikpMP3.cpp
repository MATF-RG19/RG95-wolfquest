...
// Be sure to name the function 'irrKlangPluginInit' and let the dll start with
// 'ikp'.

#ifdef WIN32
// Windows version
__declspec(dllexport) void __stdcall irrKlangPluginInit(ISoundEngine *engine,
                                                        const char *version)
#else
// Linux version
...
#endif
{
  ...
  if (strcmp(version, IRR_KLANG_VERSION)) {
    printf("This MP3 plugin only supports irrKlang version %s, mp3 playback "
           "disabled.\n",
           IRR_KLANG_VERSION);
    return;
  }
