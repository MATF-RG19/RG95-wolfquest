...
// All classes and functions of irrKlang can be found in the namespace
// 'irrklang'. If you want to use a class of the engine, you'll have to type an
// irrklang:: before the name of the class. For example, to use the
// ISoundEngine, write: irrklang::ISoundEngine. To avoid having to put
// irrklang:: before of the name of every class, we tell the compiler that we
// use that namespaces here.

using namespace irrklang;

// To be able to use the irrKlang.dll file, we need to link with the
...
// specify several options as parameters when invoking that function, but for
// this example, the default parameters are enough.
int main(int argc, const char **argv) {
  ...
  // you should delete all objects you created with a method or function that
  // starts with 'create'. (an exception is the play2D()- or play3D()-method,
