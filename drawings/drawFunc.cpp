...
void setNormalAndVertex(TACKA a, TACKA b, TACKA c) {
  ...
  glNormal3f(A.y * B.z - A.z * B.y, A.z * B.x - A.x * B.z,
             A.x * B.y - A.y * B.x);
...
void drawTerrain(void) {
  ...
  for (int i = 0; i < RAND_MATRIX_N; i++) {
    for (int j = 0; j < RAND_MATRIX_M; j++) {
      glPushMatrix();
      glTranslatef(randMatrix[i][j].x, randMatrix[i][j].z / 1.74 - 1,
                   randMatrix[i][j].z);
      glRotatef(-randMatrix[i][j].randAngle, 1, 0, 0);
      glScalef(randMatrix[i][j].randScale, randMatrix[i][j].randScale,
...
void drawObstacles(void) {
  for (int j = 0; j < RAND_OBSTACLE; j++) {
    drawObstacle(randObstacle[j].x, randObstacle[j].type,
                 randObstacle[j].position);
  }
}
